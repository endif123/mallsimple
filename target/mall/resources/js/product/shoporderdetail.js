 ...
 *//**
 ...
 $(function(){
 	...
 	var productInfoUrl = '/mall/shopping/selectOrderByShopOwner?orderId=' + orderId;//请求订单信息
 	
 	sendOderUrl="/mall/shopping/sendorderbyid?orderId=" + orderId;
 	getInfo(orderId);
 	...
 	$('#submit').click(
 			function () {
 				//检测发货状态
 				var state = IsNullReturnNull($('#sendOrNot').val());
 				if(state != "未发货"){
 					$.toast('您已发货');
 					return;
 				}
 				//如果还没发货
 				$.getJSON(sendOderUrl, function(data) {
 					if (data.success) {
 						$.toast('发货成功');
